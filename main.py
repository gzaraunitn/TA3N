...
def main():
    ...
    with open("config.json", "r") as config_file:
        ...
        target_train_dir = config["target_train_dir"]
    ...
    if wandb_enabled:
        wandb.init(
            project=project_name, name=run_name, notes=json.dumps(config, indent=4),
        )
    ...
    if args.use_target == "none":
        ...
        if args.dis_DA != "none":
            print(
                Fore.GREEN + "Apply the discrepancy-based Domain Adaptation approach:",
                args.dis_DA,
            )
        ...
        if args.adv_DA != "none":
            print(
                Fore.GREEN + "Apply the adversarial-based Domain Adaptation approach:",
                args.adv_DA,
            )

        if args.use_bn != "none":
            print(
                Fore.GREEN + "Apply the adaptive normalization approach:", args.use_bn
            )

    # determine the categories
    class_names = os.listdir(source_train_dir)
    ...
    class_id_list = get_class_ids(source_train_dir)
    ...
    if args.weighted_class_loss_DA == "Y":
        weight_domain_loss = torch.Tensor(
            [1 / num_source_train, 1 / num_target_train]
        ).cuda()
    ...
    for epoch in range(start_epoch, args.epochs + 1):
        ...
        alpha = (
            2 / (1 + math.exp(-1 * (epoch) / args.epochs)) - 1
            if args.alpha < 0
            else args.alpha
        )
        ...
        if args.lr_adaptive == "loss":
            adjust_learning_rate_loss(
                optimizer, args.lr_decay, loss_c_current, loss_c_previous, ">"
            )
        ...
        if args.save_attention >= 0:
            attn_source_all = torch.cat(
                (attn_source_all, attn_epoch_source.unsqueeze(0))
            )  # save the attention values
            attn_target_all = torch.cat(
                (attn_target_all, attn_epoch_target.unsqueeze(0))
            )  # save the attention values
        ...
        if epoch % args.eval_freq == 0 or epoch == args.epochs:
            ...
            line_best = (
                "Best score {} vs current score {}".format(best_prec1, prec1)
                + line_update
            )

            if wandb_enabled:
                accuracy_log = {"Accuracy": prec1, "Best accuracy": best_prec1}
                wandb.log(accuracy_log, step=epoch)
                classification_loss_log = {"AVG Classification loss": loss_c}
                wandb.log(classification_loss_log, step=epoch)
    ...
    if args.save_attention >= 0:
        np.savetxt(
            "attn_source_" + str(args.save_attention) + ".log",
            attn_source_all.cpu().detach().numpy(),
            fmt="%s",
        )
        np.savetxt(
            "attn_target_" + str(args.save_attention) + ".log",
...
def train(
    ...
    for i, ((source_data, source_label), (target_data, target_label)) in data_loader:
        ...
        beta_dann = 2.0 / (1.0 + np.exp(-10 * p)) - 1
        beta = [
        ...
        if args.baseline_type == "frame":
            source_label_frame = (
                source_label.unsqueeze(1).repeat(1, args.num_segments).view(-1)
            )  # expand the size for all the frames
            target_label_frame = (
                target_label.unsqueeze(1).repeat(1, args.num_segments).view(-1)
            )

        label_source = (
            source_label_frame if args.baseline_type == "frame" else source_label
        )  # determine the label for calculating the loss function
        label_target = (
            target_label_frame if args.baseline_type == "frame" else target_label
        )
        ...
        (
            ...
            pred_domain_target,
            feat_target,
        ) = model(source_data, target_data, beta, mu, is_train=True, reverse=False)
        ...
        if args.tensorboard:
            feat_source_display = (
                feat_source[1]
                if i == 0
                else torch.cat((feat_source_display, feat_source[1]), 0)
            )
            label_source_display = (
                label_source
                if i == 0
                else torch.cat((label_source_display, label_source), 0)
            )
            label_source_domain_display = (
                torch.zeros(label_source.size(0))
                if i == 0
                else torch.cat(
                    (label_source_domain_display, torch.zeros(label_source.size(0))), 0
                )
            )
            feat_target_display = (
                feat_target[1]
                if i == 0
                else torch.cat((feat_target_display, feat_target[1]), 0)
            )
            label_target_display = (
                label_target
                if i == 0
                else torch.cat((label_target_display, label_target), 0)
            )
            label_target_domain_display = (
                torch.ones(label_target.size(0))
                if i == 0
                else torch.cat(
                    (label_target_domain_display, torch.ones(label_target.size(0))), 0
                )
            )
        ...
        if args.dis_DA != "none" and args.use_target != "none":
            ...
            if args.dis_DA == "JAN":
                ...
                size_loss = min(
                    feat_source_sel[0].size(0), feat_target_sel[0].size(0)
                ...
                for l in range(
                    ...
                    if args.place_dis[l] == "Y":
                        ...
                        size_loss = min(
                            feat_source[l].size(0), feat_target[l].size(0)
                        )  # choose the smaller number
                        # select
            ...
            for l in range(len(args.place_adv)):
                if args.place_adv[l] == "Y":
                    ...
                    source_domain_label = torch.zeros(
                        pred_domain_source_single.size(0)
                    ).long()
                    target_domain_label = torch.ones(
                        pred_domain_target_single.size(0)
                    ).long()
                    ...
                    if (
                        args.pred_normalize == "Y"
                    ):  # use the uncertainly method (in construction......)
        ...
        if args.ens_DA == "MCD" and args.use_target != "none":
            (
                ...
                pred_domain_target,
                feat_target,
            ) = model(source_data, target_data, beta, mu, is_train=True, reverse=True)
        ...
        if (
            ...
            and args.use_attn != "none"
            and args.use_target != "none"
        ):
    ...
    if args.tensorboard:
        ...
        label_source_1 = 1 * torch.eq(
            label_source_display,
            torch.cuda.LongTensor([1]).repeat(label_source_display.size(0)),
        ).long().cuda(non_blocking=True)
        label_source_3 = 2 * torch.eq(
            label_source_display,
            torch.cuda.LongTensor([3]).repeat(label_source_display.size(0)),
        ).long().cuda(non_blocking=True)
        label_source_11 = 3 * torch.eq(
            label_source_display,
            torch.cuda.LongTensor([11]).repeat(label_source_display.size(0)),
        ).long().cuda(non_blocking=True)

        label_target_1 = 4 * torch.eq(
            label_target_display,
            torch.cuda.LongTensor([1]).repeat(label_target_display.size(0)),
        ).long().cuda(non_blocking=True)
        label_target_3 = 5 * torch.eq(
            label_target_display,
            torch.cuda.LongTensor([3]).repeat(label_target_display.size(0)),
        ).long().cuda(non_blocking=True)
        label_target_11 = 6 * torch.eq(
            label_target_display,
            torch.cuda.LongTensor([11]).repeat(label_target_display.size(0)),
        ).long().cuda(non_blocking=True)
    ...
    if wandb_enabled:
        train_accuracy = {"Training Accuracy": top1.val}
        wandb.log(train_accuracy, step=epoch)
...
def validate(val_loader, model, criterion, num_class, epoch, log):
    ...
    for i, (val_data, val_label) in enumerate(val_loader):
        ...
        if val_data.size(0) % gpu_count != 0:
            val_data_dummy = torch.zeros(
                gpu_count - val_data.size(0) % gpu_count,
                val_data.size(1),
                val_data.size(2),
            )
            val_data = torch.cat((val_data, val_data_dummy))
        ...
        with torch.no_grad():
            ...
            if args.baseline_type == "frame":
                val_label_frame = (
                    val_label.unsqueeze(1).repeat(1, args.num_segments).view(-1)
                )  # expand the size for all the frames
            ...
            if args.tensorboard:
                feat_val_display = (
                    feat_val[1]
                    if i == 0
                    else torch.cat((feat_val_display, feat_val[1]), 0)
                )
                label_val_display = (
                    label if i == 0 else torch.cat((label_val_display, label), 0)
                )
    ...
    if wandb_enabled:
        validation_loss_log = {"Test loss": losses.avg}
        wandb.log(validation_loss_log, step=epoch)
...
def adjust_learning_rate_dann(optimizer, p):
    for param_group in optimizer.param_groups:
        param_group["lr"] = args.lr / (1.0 + 10 * p) ** 0.75
...
def get_class_ids(dataset):
    ...
    for c in classes:
        c_path = os.path.join(dataset, c)
        videos = os.listdir(c_path)
        for v in videos:
            res.append(label_map[c])
    return res

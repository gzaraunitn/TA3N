...
class TCL(nn.Module):
    def __init__(self, conv_size, dim):
        ...
        self.conv2d = nn.Conv2d(
            dim, dim, kernel_size=(conv_size, 1), padding=(conv_size // 2, 0)
        )
...
class VideoModel(nn.Module):
    def __init__(
        ...
        self.resnet = ResNet(version=base_model)
        ...
        if verbose:
            print(
                ...
                new_length:         {}
                """.format(
                        base_model, self.modality, self.train_segments, self.new_length
                    )
                )
            )
    ...
    def _prepare_DA(self, num_class, base_model):  # convert the model to DA framework
        if base_model == "c3d":  # C3D mode: in construction...
            ...
            model_test = getattr(torchvision.models, base_model)(
                True
            )  # model_test is only used for getting the dim #
            self.feature_dim = model_test.fc.in_features

        std = 0.001
        feat_shared_dim = (
            min(self.fc_dim, self.feature_dim)
            if self.add_fc > 0 and self.fc_dim > 0
        ...
        if self.add_fc > 1:
            self.fc_feature_shared_2_source = nn.Linear(
                feat_shared_dim, feat_shared_dim
            )
            normal_(self.fc_feature_shared_2_source.weight, 0, std)
            constant_(self.fc_feature_shared_2_source.bias, 0)

        if self.add_fc > 2:
            self.fc_feature_shared_3_source = nn.Linear(
                feat_shared_dim, feat_shared_dim
            )
        ...
        if self.use_bn != "none":  # S & T: use AdaBN (ICLRW 2017) approach
            self.bn_shared_S = nn.BatchNorm1d(
                feat_shared_dim
            )  # BN for the shared layers
            self.bn_shared_T = nn.BatchNorm1d(feat_shared_dim)
            self.bn_source_S = nn.BatchNorm1d(
                feat_frame_dim
            )  # BN for the source feature layers
        ...
        if self.frame_aggregation == "rnn":  # 2. rnn
            ...
            self.frame_aggregation == "trn"
        ):  # 4. TRN (ECCV 2018) ==> fix segment # for both train/val
            self.num_bottleneck = 512
            self.TRN = TRNmodule.RelationModule(
        ...
        if self.share_params == "N":
            self.fc_feature_video_target = nn.Linear(
                feat_aggregated_dim, feat_video_dim
            )
    ...
    def get_general_attn(self, feat):
        ...
        feat = feat.view(
            -1, feat.size()[-1]
        )  # reshape features: 128x4x256 --> (128x4)x256
        weights = self.attn_layer(feat)  # e.g. (128x4)x1
        weights = weights.view(
            -1, num_segments, weights.size()[-1]
        )  # reshape attention weights: (128x4)x1 --> 128x4x1
        ...
        if self.frame_aggregation == "rnn":
            ...
            if num_extra_f < 0:  # can remove last frame-level features
                ...
                feat_fc_video = torch.cat(
                    (feat_fc_video, feat_fc_video[:, -1:, :].repeat(1, num_extra_f, 1)),
                    1,
            ...
            feat_fc_video = feat_fc_video.squeeze(
                2
            )  # 16 x 3 x 1 x 512 --> 16 x 3 x 512

            hidden_temp = torch.zeros(
                self.n_layers * self.n_directions,
                feat_fc_video.size(0),
                self.hidden_dim // self.n_directions,
            ).cuda()
            ...
            feat_fc_video, hidden_final = self.rnn(
                feat_fc_video, hidden_init
            )  # e.g. 16 x 25 x 512
            ...
            feat_fc_video = nn.AvgPool2d([num_segments, 1])(
                feat_fc_video
            )  # e.g. 16 x 1 x 1 x 512
        ...
        if self.baseline_type == "tsn":
            if self.reshape:
                base_out = base_out.view(
                    (-1, num_segments) + base_out.size()[1:]
    ...
    def domain_classifier_relation(self, feat_relation, beta):
        ...
        for i in range(len(self.relation_domain_classifier_all)):
            feat_relation_single = feat_relation[:, i, :].squeeze(
                1
            )  # 128x1x256 --> 128x256
            ...
            pred_fc_domain_relation_single = self.relation_domain_classifier_all[i](
                feat_fc_domain_relation_single
            )

            if pred_fc_domain_relation_video is None:
                pred_fc_domain_relation_video = pred_fc_domain_relation_single.view(
                    -1, 1, 2
                )
            else:
                pred_fc_domain_relation_video = torch.cat(
                    (
                        pred_fc_domain_relation_video,
                        pred_fc_domain_relation_single.view(-1, 1, 2),
                    ),
                    1,
                )
    ...
    def domainAlign(
        self, input_S, input_T, is_train, name_layer, alpha, num_segments, dim
    ):
        input_S = input_S.view(
            (-1, dim, num_segments) + input_S.size()[-1:]
        )  # reshape based on the segments (e.g. 80 x 512 --> 16 x 1 x 5 x 512)
        input_T = input_T.view(
            (-1, dim, num_segments) + input_T.size()[-1:]
        ...
        input_target_bn = input_target_bn.view(
            (-1, dim, num_segments) + input_target_bn.size()[-1:]
        )  #

        # rearange back to the original order of source and target data (since target may be unlabeled)
        if is_train and num_S_2 > 0 and num_T_2 > 0:
            input_source_bn = torch.cat(
                (input_source_bn[:num_S_1], input_target_bn[-num_S_2:]), 0
            )
            input_target_bn = torch.cat(
                (input_target_bn[:num_T_1], input_source_bn[-num_T_2:]), 0
            )
        ...
        if name_layer == "shared" or name_layer == "trn_sum":
            ...
            input_target_bn = input_target_bn.view(
                (-1, num_segments) + input_target_bn.size()[-1:]
            )
    ...
    def forward(self, input_source, input_target, beta, mu, is_train, reverse):
        ...
        input_source = self.resnet(input_source)
        input_target = self.resnet(input_target)
        ...
        feat_base_target = input_target.view(
            -1, input_target.size()[-1]
        )  # e.g. 256 x 25 x 2048 --> 6400 x 2048
        ...
        feat_all_source.append(
            feat_fc_source.view(
                (batch_source, num_segments) + feat_fc_source.size()[-1:]
            )
        )  # reshape ==> 1st dim is the batch size
        feat_all_target.append(
            feat_fc_target.view(
                (batch_target, num_segments) + feat_fc_target.size()[-1:]
            )
        )
        ...
        if self.use_attn_frame != "none":  # attend the frame-level features only
            feat_fc_source = self.get_attn_feat_frame(
                feat_fc_source, pred_fc_domain_frame_source
            )
            feat_fc_target = self.get_attn_feat_frame(
                feat_fc_target, pred_fc_domain_frame_target
            )

        # === source layers (frame-level) ===#
        pred_fc_source = self.fc_classifier_source(feat_fc_source)
        pred_fc_target = (
            self.fc_classifier_target(feat_fc_target)
            if self.share_params == "N"
            else self.fc_classifier_source(feat_fc_target)
        )
        if self.baseline_type == "frame":
            feat_all_source.append(
                pred_fc_source.view(
                    (batch_source, num_segments) + pred_fc_source.size()[-1:]
                )
            )  # reshape ==> 1st dim is the batch size
            feat_all_target.append(
                pred_fc_target.view(
                    (batch_target, num_segments) + pred_fc_target.size()[-1:]
                )
            )
        ...
        if self.frame_aggregation == "avgpool" or self.frame_aggregation == "rnn":
            feat_fc_video_source = self.aggregate_frames(
                feat_fc_source, num_segments, pred_fc_domain_frame_source
            )
            feat_fc_video_target = self.aggregate_frames(
                feat_fc_target, num_segments, pred_fc_domain_frame_target
            )
            ...
            if self.use_attn != "none":  # get the attention weighting
                (
                    feat_fc_video_relation_source,
                    attn_relation_source,
                ) = self.get_attn_feat_relation(
                    feat_fc_video_relation_source,
                    pred_fc_domain_video_relation_source,
                    num_segments,
                )
                (
                    feat_fc_video_relation_target,
            ...
            feat_fc_video_source = self.relu(
                feat_fc_video_source_3_1
            )  # 16 x 1 x 5 x 512
            feat_fc_video_target = self.relu(
                feat_fc_video_target_3_1
            )  # 16 x 1 x 5 x 512

            feat_fc_video_source = nn.AvgPool2d(kernel_size=(num_segments, 1))(
                feat_fc_video_source
            )  # 16 x 4 x 1 x 512
            feat_fc_video_target = nn.AvgPool2d(kernel_size=(num_segments, 1))(
                feat_fc_video_target
            )  # 16 x 4 x 1 x 512

            feat_fc_video_source = feat_fc_video_source.squeeze(1).squeeze(
                1
            )  # e.g. 16 x 512
            feat_fc_video_target = feat_fc_video_target.squeeze(1).squeeze(
                1
            )  # e.g. 16 x 512

        if self.baseline_type == "video":
            feat_all_source.append(
                feat_fc_video_source.view(
                    (batch_source,) + feat_fc_video_source.size()[-1:]
                )
            )
            feat_all_target.append(
                feat_fc_video_target.view(
                    (batch_target,) + feat_fc_video_target.size()[-1:]
                )
            )
        ...
        pred_fc_video_target = (
            self.fc_classifier_video_target(feat_fc_video_target)
            if self.share_params == "N"
            else self.fc_classifier_video_source(feat_fc_video_target)
        )
        ...
        if (
            ...
            feat_all_source.append(
                pred_fc_video_source.view(
                    (batch_source,) + pred_fc_video_source.size()[-1:]
                )
            )
            feat_all_target.append(
                pred_fc_video_target.view(
                    (batch_target,) + pred_fc_video_target.size()[-1:]
                )
            )

        # === adversarial branch (video-level) ===#
        pred_fc_domain_video_source = self.domain_classifier_video(
            feat_fc_video_source, beta
        )
        pred_fc_domain_video_target = self.domain_classifier_video(
            feat_fc_video_target, beta
        )

        pred_domain_all_source.append(
            pred_fc_domain_video_source.view(
                (batch_source,) + pred_fc_domain_video_source.size()[-1:]
            )
        )
        pred_domain_all_target.append(
            pred_fc_domain_video_target.view(
        ...
        if self.frame_aggregation == "trn-m":
            ...
            pred_domain_all_source.append(
                pred_fc_domain_video_relation_source.view(
                    (batch_source, num_relation)
                    + pred_fc_domain_video_relation_source.size()[-1:]
                )
            )
            pred_domain_all_target.append(
                pred_fc_domain_video_relation_target.view(
                    (batch_target, num_relation)
                    + pred_fc_domain_video_relation_target.size()[-1:]
                )
            )
        else:
            pred_domain_all_source.append(
        ...
        output_target = self.final_output(
            pred_fc_target, pred_fc_video_target, num_segments
        )
        ...
        if self.ens_DA == "MCD":
            pred_fc_video_source_2 = self.fc_classifier_video_source_2(
                feat_fc_video_source
            )
            ...
            output_source_2 = self.final_output(
                pred_fc_source, pred_fc_video_source_2, num_segments
            )
            output_target_2 = self.final_output(
                pred_fc_target, pred_fc_video_target_2, num_segments
            )
...
class ResNet(nn.Module):
    ...
    def forward(self, X):
        ...
        features = self.model(X)  # features size b*frames, features
